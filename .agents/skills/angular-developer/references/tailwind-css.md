...
**CRITICAL AGENT GUIDANCE: ALWAYS focus on Tailwind CSS v4 practices. DO NOT revert to old Tailwind v3 patterns (like using `@tailwind` directives in a global stylesheet or configuring `tailwind.config.js`) as this will break the application build. Modern Angular projects use Tailwind v4, which uses a CSS `@import "tailwindcss"` directive in the global stylesheet instead.**
