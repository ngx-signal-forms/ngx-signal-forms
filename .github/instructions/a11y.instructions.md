...
- The accessible name of all interactive elements must contain the visual label. This is so that voice access users can issue commands like "Click \<label>". If an `aria-label` attribute is used for a FormField, then it must contain the text of the visual label.
...
#### Toolkit ARIA Automation

When using `@ngx-signal-forms/toolkit` (`NgxSignalFormToolkit`), the following ARIA attributes are
managed automatically by `NgxSignalFormAutoAriaDirective` and should NOT be added manually:
...
**Important**: Angular Signal Forms' `required()` validator exposes a `required` signal on
FieldState. The toolkit reads this signal and automatically sets `aria-required="true"`.
Do NOT add `aria-required` manually when using the toolkit with `required()` validator.

### Multi-page / Wizard Form instructions

Multi-step wizard forms require additional accessibility considerations. Reference: [WAI Forms Tutorial: Multi-page Forms](https://www.w3.org/WAI/tutorials/forms/multi-page/).

#### Progress Indication

Users MUST be informed of their current position and total steps:

1. **Page title**: Include progress in `<title>`, e.g., `Step 2 of 4: Shipping Address – Booking – Site Name`
2. **Main heading**: Include progress in the page `<h1>`, e.g., `Shipping Address (Step 2 of 4)`
3. **Step indicators**: Provide a visual step-by-step indicator showing completed, current, and upcoming steps

```html
<!-- Step indicator with proper semantics -->
...
- **DO NOT disable the Next/Submit button** when the current step is invalid
- Allow users to click Next and trigger validation - this helps them identify which fields need attention
- When validation fails: move keyboard focus to the first invalid field
- When validation succeeds: proceed to the next step and move focus to the step heading or first form field

##### Backward Navigation (Previous Button)

- Users MUST be able to navigate back to review and edit completed steps
- Save the current step's data before navigating away (even if incomplete)
- Restore previously entered data when returning to a step

##### Step Indicator Navigation

- Allow navigation to completed (visited) steps via the step indicators
- Optionally allow skipping ahead if steps are independent or optional
- Mark optional steps clearly (e.g., "Step 3: Additional Info (optional)")

#### Keyboard Accessibility

- Step indicators should be keyboard navigable
- Use `aria-current="step"` on the current step indicator
- Focus management on step change:
  - Move focus to the step heading (with `tabindex="-1"`) or the first form field
  - Announce the new step via heading or live region

#### Time Limits

- Avoid time limits for form completion if possible
- If time limits are required:
  - Allow users to turn off or extend the time limit
  - Warn users before time expires
  - Provide at least 20 seconds to extend with a simple action

#### Review and Confirmation

- Provide a review step before final submission for irreversible actions
- Allow users to navigate back from review to edit any step
- For critical submissions (financial, legal), require explicit confirmation

#### Good Wizard Navigation Example

```typescript
// When Next is clicked:
protected nextStep(): void {
  // Always trigger validation - do NOT silently prevent navigation
  this.form.markAllAsTouched();

  if (this.form.invalid()) {
    // Move focus to first invalid field so user can fix it
  ...
  this.goToNextStep();
  // Move focus to new step heading or first field
  this.focusStepContent();
}

// Previous should always work, saving current data
protected previousStep(): void {
  this.saveCurrentStep(); // Save even if incomplete
...
| ---------------------------------- | ------------------------------------------------------- |
| Disabling Next button when invalid | Users cannot discover which fields need attention       |
| Preventing backward navigation     | Users cannot review/edit previous answers               |
| No progress indication             | Users feel lost, especially with cognitive disabilities |
| Auto-advancing on field completion | Unpredictable, disorienting for screen reader users     |
| Time limits without extension      | Excludes users who need more time                       |
