...
| `generateErrorId(fieldName, kind?)`                       | Derive `{fieldName}-error` (container) or `{fieldName}-error-{kind}` (per-error) element id. Turns inner whitespace in `fieldName` into `-`, so the id stays one `aria-describedby` token. |
| `generateWarningId(fieldName)`                            | Derive the `{fieldName}-warning` element id used for `aria-describedby`. Turns inner whitespace in `fieldName` into `-`.                                                                   |
