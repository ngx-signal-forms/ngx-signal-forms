...
describe('ngxSignalFormDirective', () => {
  ...
  describe('Submission state tracking (user-facing behavior)', () => {
    ...
    it('should track submission lifecycle with submit() helper', async () => {
      @Component({
        ...
        imports: [Field, NgxSignalFormDirective, ContextDisplayComponent],
        template: `
  ...
  describe('Nested forms', () => {
    it('should support nested forms with independent contexts', async () => {
      ...
      @Component({
        selector: 'ngx-test-nested-forms',
        imports: [NgxSignalFormDirective, OuterDisplay, InnerDisplay],
        template: `
          <form [ngxSignalForm]="outerForm" [errorStrategy]="'on-touch'">
            <ngx-signal-form-outer-display />
            <div [ngxSignalForm]="innerForm" [errorStrategy]="'immediate'">
      ...
      class TestComponent {
        readonly #outerModel = signal({ name: '' });
        readonly outerForm = form(this.#outerModel);
        readonly #innerModel = signal({ email: '' });
        readonly innerForm = form(this.#innerModel);
      }
