...
test.describe('Warning Support Demo', () => {
  ...
  test.fixme(
    'should NOT show errors on initial load (CRITICAL BUG CHECK)',
  ...
  test.describe('Warnings vs Errors', () => {
    ...
    test.fixme(
      'should allow submission with warnings (non-blocking)',
      async () => {
  ...
  test.describe('Form Validation', () => {
    test('should validate required fields on submit', async () => {
      ...
      await test.step('Verify NO errors appear (on-touch mode requires blur first)', async () => {
        // In on-touch mode, errors don't show until field is touched
        // Submitting alone doesn't trigger error display
        const alerts = page.page.locator('[role="alert"]');
        await expect(alerts).toHaveCount(0);
      });

      await test.step('Touch a field to trigger error display', async () => {
        await page.usernameInput.focus();
        await page.usernameInput.blur();
  ...
  test.describe('Form Submission', () => {
    test.fixme(
      'should show success message after valid submission (no warnings)',
      async () => {
        /**
